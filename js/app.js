...
if (readCookie("_sales24JWT") != "") {
  ...
  (document.getElementById("userActions").innerHTML =
    "Hi, " + user_data.fullName),
    (document.getElementById("lastLogin").innerHTML =
      "Last Login: " + user_data.lastLogin);
}
...
function logout() {
  ...
  if (logout_confirm) {
    const url =
      "https://8xmv9zllk6.execute-api.us-east-1.amazonaws.com/DEV/me/logout";
    document.getElementById("loader").style.display = "block";
    var logout_operation = async () =>
      await fetch(url, {
...
function logoutAll() {
  ...
  if (logoutAll_confirm) {
    const url =
      "https://8xmv9zllk6.execute-api.us-east-1.amazonaws.com/DEV/me/logoutall";
    document.getElementById("loader").style.display = "block";
    var logoutAll_operation = async () =>
      await fetch(url, {
        Host: "8xmv9zllk6.execute-api.us-east-1.amazonaws.com",
        method: "GET",
        headers: {
...
var generateCard = async function(
  id,
  name,
  desc,
  price,
  ...
  await fetch(url, {
    ...
    .then(res => {
      ...
      return res.text().then(data => {
        ...
        document.getElementById(id + "/totalLikes").innerText =
          " " + data.likes;
...
var recentlyAdsPosted = async () => {
  ...
  var url =
    "https://8xmv9zllk6.execute-api.us-east-1.amazonaws.com/DEV/me/recentlypostedads";
      ...
      return res.text().then(async data => {
        ...
        if (total_data.LAST_KEY) {
          sessionStorage.setItem(
            "lastSearchedKey",
            JSON.stringify(total_data.LAST_KEY)
          );
      ...
      return res.text().then(async data => {
        ...
        for (var i = 0; i < total_data.length; i++) {
          ...
          if (i == 3) {
            ...
            document.getElementById("loaderCategory").style.display = "none";
            if (total_data.length > i) {
              document.getElementById("searchMoreCategory").style.display =
                "block";
...
var likedADS = async function(ad_key) {
  var url =
    "https://8xmv9zllk6.execute-api.us-east-1.amazonaws.com/DEV/me/likedads";
